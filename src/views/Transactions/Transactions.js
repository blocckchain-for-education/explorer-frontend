...
function TransactionRow(props) {
  ...
  return (
    <tr key={transaction.header_signature.toString()}>
      <th scope="row"><Link to={txLink}>{transaction.header_signature.substring(0,15)}...</Link></th>
      <td><Link to={txLink}>{transaction.header.family_name}</Link></td>
      ...
      <td><Link to={txLink}><Badge color={getBadge(transaction.status)}>{transaction.status}</Badge></Link></td>
    </tr>
  )
}
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
            {/* <Card> */}
              {/* <CardHeader>
                <i className="fa fa-align-justify"></i> Transactions <small className="text-muted">example</small>
              </CardHeader> */}
              {/* <CardBody> */}
